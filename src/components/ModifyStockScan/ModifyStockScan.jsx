...
const ModifyStockScan = ({ modifyMode, onSetNewStock, setModifyMode }) => {
    ...
    useEffect(() => {
        if(productsScanned && barcodesError?.length > 0) {
        ...
        console.log(products, productsScanned,barcodesError);
    ...
    console.log('barcodesScanned', barcodesScanned)
    console.log('productsScanned', productsScanned)

    return (
        <div className={`${style.container} selectMode`}>
            <h4> Scaneaza</h4>
            <textarea ref={textAreaRef} autofocus placeholder='Scaneaza produsele' value={barcodesScanned} onChange={e => onScanned(e)} />
            {/* {scanMode
                ? <Button variant="contained" color="success" onClick={() => onScanCompleted()}>  Aplica </Button>
                : (<> */}
                    <Button variant="contained" color={scanMode === 1 ? "success" : "inherit"} onClick={() => onSelectScanMode(1)} disabled={modifyMode}> Adauga </Button>
                    <Button variant="contained" color={scanMode === 1? "inherit" : "error"} onClick={() => onSelectScanMode(-1)} disabled={modifyMode}> Scoate </Button>
                {/* </>)} */}
            {scanMode && barcodesScanned 
                ? ( 
                    <div>
                        <h3> Produse scanate </h3>
                        <div className='flex'>
